...
import { Clock, Calendar, ChevronRight, Activity, Search, Shield } from "lucide-react";
import { useSearch } from "../../contexts/SearchContext";
import { useSubjects } from "../../contexts/SubjectsContext";
import { formatLocalDate, formatLocalTime } from "../../utils/dateUtils";

export default function TeacherRecentSessionsPage() {
        ...
        <div className="max-w-6xl mx-auto pb-10">
            <header className="mb-10">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-indigo-600 font-bold text-sm uppercase tracking-wider">Teacher Archives</span>
                </div>
                <h1 className="text-3xl font-bold text-soft-900 mb-2">Global Class History</h1>
                    ...
                    {filteredSessions.map((s, i) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className="bg-white p-6 rounded-[2rem] border border-soft-100 shadow-soft hover:border-indigo-200 hover:shadow-soft-xl cursor-pointer group soft-transition relative overflow-hidden"
                            >
                                <div className="flex items-center gap-6">
                                    <div className="w-16 h-16 rounded-2xl bg-soft-50 flex flex-col items-center justify-center border border-soft-100 group-hover:bg-indigo-50 group-hover:border-indigo-100 soft-transition">
                                        <span className="text-[10px] font-bold text-soft-400 uppercase tracking-tighter">{month}</span>
                                        <span className="text-2xl font-black text-soft-900 group-hover:text-indigo-600 leading-none">{day}</span>
                                    </div>

                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                            <div className={`w-1.5 h-1.5 rounded-full ${s.status === 'active' ? 'bg-indigo-500 animate-pulse' : 'bg-green-500'}`}></div>
                                            <span className="text-[10px] font-bold text-soft-400 uppercase tracking-[0.2em]">{s.subject_name}</span>
                                        </div>
                                        <h3 className="text-xl font-bold text-soft-900 group-hover:text-indigo-500 soft-transition">
                                    ...
                                    <div className="p-3 rounded-full bg-soft-50 text-soft-300 group-hover:bg-indigo-600 group-hover:text-white soft-transition">
                                        <ChevronRight className="w-6 h-6" />
                                    </div>
                                </div>
                                <div className="absolute top-0 right-10 w-24 h-full bg-indigo-600/5 skew-x-[-30deg] translate-x-20 group-hover:translate-x-0 transition-transform duration-700 pointer-events-none"></div>
                            </motion.div>
                        );
                    })}
            ...
            {!loading && hasMore && sessions.length > 0 && (
                <div className="mt-12 flex justify-center">
                    <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                        onClick={loadMore}
                        className="bg-white border-2 border-indigo-100 text-indigo-600 px-8 py-4 rounded-2xl font-bold shadow-soft hover:bg-indigo-50 hover:border-indigo-200 soft-transition flex items-center gap-2"
                    >
                        Load More Sessions
